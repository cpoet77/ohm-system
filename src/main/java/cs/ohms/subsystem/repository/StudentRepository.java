...
public interface StudentRepository extends JpaRepository<StudentEntity, String>, JpaSpecificationExecutor<StudentEntity> {
    ...
    @Query(value = "SELECT COUNT(student_id) count FROM ohms_view_student WHERE student_id IN (SELECT student_id FROM ohms_student_course_group WHERE course_group_id = :courseGroupId)", nativeQuery = true)
